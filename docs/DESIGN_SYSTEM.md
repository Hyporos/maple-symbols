...
**Page layouts** (`src/next/shell/`, `src/next/pages/`): `NextShell` is the /next frame — a `sticky top-0 z-30` wrapper around `NextHeader` and `SuggestionBanner` (so the banner docks `top-full` under the header, DESIGN_SYSTEM §6), `<main id="main" className="mx-auto w-full max-w-[1200px] flex-1 px-4 pb-16 md:px-8">` for the page, a floating `FeedbackButton` (`fixed right-4 bottom-20 z-30`, `md:bottom-6`; a `size-11` round icon button below 1440 px so it fits the gutter without covering a card, its label `sr-only` there and shown from 1440 px; `aria-haspopup="dialog"`, opens a `ComingSoonNote` since reporting a wrong number is a later 2.0 feature), and `NextFooter` (a plain `border-t border-white/6` strip, not the gradient panel). `NextHeader` is `border-b border-white/6 bg-linear-to-t from-card to-card-grad`, `h-16`; on phones the nav, `NextServerMenu` and `AccessibilityButton` collapse into a `☰` menu while `CharacterChip` stays in the bar.

`CalculatorPage`: from 768 px, `grid gap-6 min-[768px]:grid-cols-[minmax(0,5fr)_minmax(0,7fr)] min-[1150px]:grid-cols-[400px_1fr]` — `SymbolPicker` + `CalculatorCard` in the left column (`min-[1150px]:sticky min-[1150px]:top-24`), `OverviewCard` + `GraphCard` stacked in the right (`min-w-0` so the chart can shrink). Below 1150 px `OverviewCard` is compact: each row's symbol is its icon alone, the name kept as `sr-only`. On phones all three card groups stay mounted (each a `role="tabpanel"` toggled by `hidden`, not conditionally rendered) behind a `BottomTabBar` (Edit/Overview/Graph). `NextApp` passes `bottomBar` to `NextShell` for this page, and on phones the shell root then gets `pb-[calc(57px+env(safe-area-inset-bottom))]` (the bar's height plus its safe-area padding), so at max scroll the footer ends right above the bar.
