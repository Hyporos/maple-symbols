...
- **NumberField**: a centred number input, `rounded-lg bg-secondary` idle, `bg-hover text-primary` on hover/focus; NaN renders as `""`; the mouse wheel blurs it so scrolling the page cannot change the value. No outline utility, so keyboard focus shows the §9 accent ring.
...
- **Tabs**: a `role="tablist"` strip, `flex border-b border-white/8`; the selected tab gets an inset accent underline (`shadow-[inset_0_-2px_0_var(--color-accent)]`). Panel ids follow `${idPrefix}-panel-${value}`; callers render only the selected tab's panel, so only the selected tab carries `aria-controls`.
...
**Page layouts** (`src/next/shell/`, `src/next/pages/`): `NextShell` is the /next frame — a `sticky top-0 z-30` wrapper around `NextHeader`, then `NextSuggestionBanner` (/next's own copy of the §6 suggestion bar: same copy, dismissal and analytics, a `nextHref` link that stays on the same /next page, and in the page's flow as a plain `flex justify-center bg-light` row under the header, so it pushes the page down instead of covering the first card and scrolls away), `<main id="main" className="mx-auto w-full max-w-[1200px] flex-1 px-4 pb-16 md:px-8">` for the page, the `FeedbackButton` (`aria-haspopup="dialog"`, opens a `ComingSoonNote` since reporting a wrong number is a later 2.0 feature), whose place depends on the width (`useFeedbackInHeader` in `src/next/shell/useFeedbackInHeader.ts`, which names the width once as `FLOATING_FEEDBACK_MIN_WIDTH`, 1256 px): from 1256 px it floats (`fixed bottom-6 z-30`, a `size-11` round icon button, labelled from 1440 px) in a gutter wide enough to hold it clear of the cards, `right-[min(1rem,calc((100%-1136px)/4-22px))]` centring it in that gutter near 1256 px so a classic scrollbar (which the media query counts but the layout loses) cannot push it onto a card; below 1256 px it is a header chip in `NextHeader`'s action cluster beside `NextServerMenu` and "Aa" (the same `rounded-lg bg-light px-3 py-2` chip, icon plus an `sr-only` "Feedback"), and on phones an entry in the ☰ menu beside them, so it never covers a card or the phone footer, and `NextFooter` (a plain `border-t border-white/6` strip, not the gradient panel). `NextHeader` is `border-b border-white/6 bg-linear-to-t from-card to-card-grad`, `h-16`; on phones the nav, `NextServerMenu` and `AccessibilityButton` collapse into a `☰` menu while `CharacterChip` stays in the bar.

`CalculatorPage`: from 768 px, `grid gap-6 min-[768px]:grid-cols-[minmax(0,5fr)_minmax(0,7fr)] min-[1150px]:grid-cols-[400px_1fr]` — `SymbolPicker` + `CalculatorCard` in the left column (`min-[1150px]:sticky min-[1150px]:top-24`), `OverviewCard` + `GraphCard` stacked in the right (`min-w-0` so the chart can shrink). On tablets (768–1149 px, `isTablet && !isMobile`) `OverviewCard` is compact: each row's symbol is its icon alone, the name kept as `sr-only`; phones show icon and name. Symbol names are `<p className="text-primary">` (a bare `<span>` is accent, §9, and read as a link); an unset symbol's target is the picker's `–`. The target-level box puts its label and `NumberField` on one line and shows the three results as `StatBox`es only once a target is entered, otherwise one hint line. `GraphCard`'s boxes are captioned with the family's power name (`graph.power`, `graph.targetPowerFull`), and its "x / max" and the picker's share one maximum, the whole family at max level (`src/next/calculator/familyMaxPower.ts`). On phones all three card groups stay mounted (each a `role="tabpanel"` toggled by `hidden`, not conditionally rendered) behind a `BottomTabBar` (Edit/Overview/Graph). `NextApp` passes `bottomBar` to `NextShell` for this page, and on phones the shell root then gets `pb-[calc(57px+env(safe-area-inset-bottom))]` (the bar's height plus its safe-area padding), so at max scroll the footer ends right above the bar.
