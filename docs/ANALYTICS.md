...
- **On `v2` only:** the typed wrapper and every §3 event, wired and tested (`src/test/analyticsEvents.test.tsx`), the server events included (2026-09-23). They reach production when `v2` ships. `error_shown` is wired through the ErrorBoundary `onError` but has no test, because that needs a render crash inside App.
...
| `mode_switch`   | `to`: `arcane` \| `sacred` (\| `grand` on /next, which has a Grand tab) | The arcane/sacred split, which decides whose bugs and content get priority.                                                                                                                                         |
| `symbol_select` | `symbol` (name), `mode` (`grand` possible on /next)                     | Which regions people are actually grinding. Drives which symbol gets attention when game data changes. Symbol 8 was `Hotel Arcus` until 2.0 and is `Arcus` from then on: count both when reading across the change. |

### Feature usage — what to invest in, and what to cut

| Event             | Data                                                                | The decision it informs                                                                                                                                                                                                               |
| ----------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tool_used`       | `tool`: `selector` \| `catalyst`; `action`: `preview` \| `apply`    | Whether the Tools card earns its space, and whether people trust Apply or just look. `/` sends `preview` when the shown tool changes; /next sends it on every opening of a tool sheet, so compare the two per opening, not per visit. |
...
### Servers — do the editions and the server switch earn their place? (REGIONS D-17, 2026-09-23)

Values are server codes (`gms`, `msea`, `kms`, `jms`, `tms`, `cms`), never names or translated labels. Which edition a visitor lands on already comes free from the pageview path (`/kms/…`).

| Event             | Data                                                                         | The decision it informs                                                                                                                                                                                                                                                                                                                                                            |
| ----------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `region_switch`   | `page`: the page's own server; `to`: the one picked under "Numbers from"     | Whether players compare servers' numbers, and which pairs (GMS players checking KMS for "coming to GMS" changes argues for REGIONS §7.4). Near zero means the "Numbers from" half of the menu is clutter and can go; a lot of `to` = the page's own server means people get lost in an override and it needs a clearer way back.                                                   |
| `edition_switch`  | `from`, `to`: a "Site version" link to another edition (not the current one) | Whether visitors find their edition through the menu, and from where. High GMS → KMS/MSEA traffic says search sends them to the wrong edition (fix hreflang or titles, SEO §2); near zero with healthy edition pageviews says search already lands them right. Sent with keepalive (the tracker's own fetch option, checked 2026-09-23), so the full page load does not cancel it. |
| `edition_suggest` | `action`: `shown` (once per page load)                                       | `clicked`                                                                                                                                                                                                                                                                                                                                                                          | `dismissed`; `to`: the suggested server | Whether the suggestion banner (D-9) earns its space: clicked ÷ shown is its hit rate. A low rate with many dismissals means the detection rules guess wrong (which `to` fails says which rule) or the bar annoys; a high rate means editions are mis-served by search and the banner is doing SEO's job. `shown` repeats on each page load until answered, so read unique visitors, not raw counts. |
