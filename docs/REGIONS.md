...
- **Korean draft** (2026-09-23): `src/i18n/ko/`, registered in `DRAFT_CATALOGUES` (not served), awaiting a native KMS player's review (D-12). `TERMS["ko"]` and `GAME_NAMES.ko` are from maplestory.nexon.com; no term is UNCONFIRMED, but `symbolSelector` (심볼 교환권, the guide's generic wording) needs a player's eye.
- **ja draft** (2026-09-23): `src/i18n/ja/`, registered in `DRAFT_CATALOGUES` (`src/i18n/drafts.ts`), not served, awaiting a native JMS player's review. `TERMS["ja"]` is complete from maplestory.nexon.co.jp notices and the Force guide (sources beside the table); no term is UNCONFIRMED.
...
- **zh-Hans draft** (2026-09-23): `src/i18n/zh-Hans/`, in `DRAFT_CATALOGUES`, not served until a native CMS player reviews it (D-12). `TERMS["zh-Hans"]` and the `zh-Hans` names come from official mxd.web.sdo.com pages (sources beside each table); UNCONFIRMED: 神秘催化剂 (arcaneCatalyst) and 选择型徽章交换券 (symbolSelector), both built from confirmed names.
