...
- **Every other design decision**, made the same day (the §9 numbers):
  ...
  - D-12 **Machine draft plus a native player's review** for interface text; game names and terms only from the official clients. The review happens on **Vercel previews** (Brian, 2026-09-23): a preview build serves the drafts (`SERVES_DRAFTS` in `src/lib/routes.ts`, from `VERCEL_ENV=preview`), production never does and its bundle leaves them out. A reviewer opens `/kms`, `/jms`, `/tms` or `/cms` on any `v2` preview link. Publishing a language moves it from `DRAFT_LANGUAGES` to `PUBLISHED_LANGUAGES`. **Published for 2.0** (Brian, 2026-09-23): there are no native KMS, JMS, TMS or CMS reviewers, so the four catalogues (`src/i18n/ko`, `ja`, `zh-Hant`, `zh-Hans`) are in `PUBLISHED_LANGUAGES` and those editions launch in their own language, indexable, with hreflang and sitemaps; players report what reads wrong. Every game term and name is from official sources; only the interface sentences are machine-drafted. `DRAFT_LANGUAGES` is empty, kept for a future language (served on Vercel previews only).
  ...
  - D-18 **Grand Sacred gets its own `grand` type in the data**; where it sits in the interface is settled with the 2.0 visual design. The data model is built (2026-09-23, GAME §4): ids 13 and 14, `null` for the rules they lack, never a `mode`, so the pages are unchanged. When the interface shows them, **their power counts toward the Sacred Power total** (Selector, Graph, Handbook ratios), as in game (Brian, 2026-09-23); `usePower` sums one family today.
...
- Grand Sacred (built 2026-09-23): `SymbolType` gains "grand" (`Mode` does not); Sacred EXP table; max 11; power +10; no main stat (a bonus line instead, not modelled yet); no Catalyst; the Sacred Symbol Selector covers Tallahart (Brian, in game, 2026-09-22) but not Geardock (`"selector": false`). Per server: MSEA's Geardrock pays 10 (an override), and a `mesosGrand` status covers the two cost tables. What the per-server `status` cannot say yet, because it covers a kind of number for the whole server: MSEA's Talahart count and the TMS and CMS Geardock counts are unpublished (GAME §5). The per-symbol `{ value, status, source, since }` above would.
...
- **Korean draft** (2026-09-23): `src/i18n/ko/`, registered in `DRAFT_CATALOGUES` (published for 2.0 on 2026-09-23), awaiting a native KMS player's review (D-12). `TERMS["ko"]` and `GAME_NAMES.ko` are from maplestory.nexon.com; no term is UNCONFIRMED, but `symbolSelector` (심볼 교환권, the guide's generic wording) needs a player's eye.
- **ja draft** (2026-09-23): `src/i18n/ja/`, registered in `DRAFT_CATALOGUES` (`src/i18n/drafts.ts`), published for 2.0 on 2026-09-23, awaiting a native JMS player's review. `TERMS["ja"]` is complete from maplestory.nexon.co.jp notices and the Force guide (sources beside the table); no term is UNCONFIRMED.
- **zh-Hant draft** (2026-09-23): `src/i18n/zh-Hant/`, published for 2.0 on 2026-09-23, awaiting a native TMS player's review (D-12); its terms are in `TERMS["zh-Hant"]` from beanfun notices. UNCONFIRMED: 祕法 and 真實 as labels on their own, 祕法觸媒 (players' word; no notice found), and 一般伺服器 as the Catalyst's restriction label.
- **zh-Hans draft** (2026-09-23): `src/i18n/zh-Hans/`, in `DRAFT_CATALOGUES`, published for 2.0 on 2026-09-23 until a native CMS player reviews it (D-12). `TERMS["zh-Hans"]` and the `zh-Hans` names come from official mxd.web.sdo.com pages (sources beside each table); UNCONFIRMED: 神秘催化剂 (arcaneCatalyst) and 选择型徽章交换券 (symbolSelector), both built from confirmed names.
- **Every term checked against official sources, 2026-09-23** (this supersedes the UNCONFIRMED lists above): MSEA all confirmed from maplesea.com (825 notices; the world type is "Normal World", not "Regular Server"); TMS 祕法觸媒 (bulletin 61423), the selector tickets and 一般伺服器 confirmed; CMS's generic selector corrected to 选择徽章交换券 (notice 379768). Two stay on player sources with Brian's agreement (change only if a player reports them): CMS 神秘催化剂, and the bare type words 祕法/真實 and 神秘/原初, which official text uses only inside compounds such as 神秘/原初徽章. The Catalyst's "[… Only]" tag now shows only on GMS, the one server with a Reboot-type world left (KMS, JMS and TMS converted theirs in 2024–25; `catalystRegularWorldOnly`, GAME §5).
...
- **Decided (Brian, 2026-09-23), the suggestion banner:** keep the full-width `bg-light` strip under the header; its copy is in the **suggested** edition's served language (`editionMessages(target)`, with `lang` on the bar), so a Korean browser on the GMS page reads Korean once KMS is published; following the link or dismissing both silence that suggestion; an Asian server's time zone outranks a GMS English (a Singaporean on en-US is offered MSEA). zh-SG/zh-MY → MSEA and the Hong Kong/Macau zones → TMS stay.
- **Decided (Brian, 2026-09-23), the reset hint:** it stays in the Calculator's day-count tooltip only (not the Overview), and stays hidden when the visitor's day turns over with the game's.
