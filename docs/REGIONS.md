...
- **Every other design decision**, made the same day (the §9 numbers):
  ...
  - D-18 **Grand Sacred gets its own `grand` type in the data**; where it sits in the interface is settled with the 2.0 visual design. The data model is built (2026-09-23, GAME §4): ids 13 and 14, `null` for the rules they lack, never a `mode`, so the pages are unchanged.
