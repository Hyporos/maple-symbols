...
**Checked, because it has moved before:** daily symbols per region; whether an extra quest exists and what it raises the daily to; the weekly (per clear × clears); the daily, weekly and extra quest names (a replaced quest usually means its reward changed); every per-level meso cost of all 12 symbols (the 2023 reduction, KMS's 30 % arcane cut in 2026); the per-level gains in power, main stat, Demon Avenger HP and Xenon stat (KMS raised Xenon's in 2026); the symbol lists for Arcane, Sacred and Grand Sacred (a new region); and the Grand Sacred daily and costs, held in the script's `PENDING` list until 2.0 adds them to `symbols.json`. The weekly and the class gains are read from the GMS profile in `src/lib/regions.json`, and the weekly is compared as the game pays it (per clear × clears), so a move to KMS's single clear of 240 is caught even though the total stays the same.
