...
- Config: `vitest.config.ts` (standalone, `plugins: [react()]`, `environment: jsdom`, `setupFiles: src/test/setup.ts`, `env: { TZ: "UTC" }` so every machine runs in CI's time zone (the suggestion banner and the local reset hint read it), `css: false`, `include: src/**/*.test.{ts,tsx}`). It is deliberately not merged with `vite.config.ts` so build-only plugins (the routes plugin) never run.
...
| `seedSymbol(id, patch, select = true)`           | patch one symbol by id (1–6 arcane, 7–12 sacred, 13–14 grand) and select it via `selectSymbol` (switches `mode` too; a grand id is patched but never selected)      |
...
**A /next page**: `renderNextAt(path)` (`src/next/testing.tsx`) sets `window.history` to `path` and renders the whole `<App/>` inside a `<RouterProvider>` — the same route `App.tsx` itself takes to hand off to `NextApp` (ARCHITECTURE §2), rather than mounting a `src/next/` component standalone. `NextApp` is lazy-loaded, so await `screen.findByTestId("next-app")` before asserting on anything inside it. Used by `src/next/NextApp.test.tsx` (the hand-off itself, and folding a Grand selection back to Sacred on leaving), `src/next/NextApp.production.test.tsx` (`NEXT_UI` mocked `false`: `/next` is then just an unknown path), and each page's colocated test in `src/next/pages/`.
...
**Time**: anything touching `dayjs()` or `new Date()` (`advanceDayCount`, `calculateDaysRemaining`, completion dates, Overview's target date, Graph, Footer's `new Date().getFullYear()`) must freeze the clock: `vi.setSystemTime(WED)`. That alone mocks `Date` without enabling fake timers, which keeps RTL's `waitFor` working. Always use the local-time `Date` fixtures; `new Date("2026-09-16")` is UTC midnight and lands on the previous evening in the Americas, shifting the weekday. Day counts read the server's game day (`gameToday()`, 00:00 UTC for GMS), and the fixtures' 10:00 local time is the same UTC date on any machine within ten hours of UTC. To test the clock itself, pass instants to `gameToday(region, dayjs(new Date("…Z")))` as `src/lib/regions.test.ts` does.
...
- **Selection is by id and tied to `mode`.** `seedSymbol(id, …)` calls `selectSymbol`, which also switches `mode` to that symbol's type; set `mode`/`selectedId` directly only when you want them deliberately out of sync. Grand Sacred ids (13, 14) are never selected and no card lists them: `src/test/grandHidden.test.tsx` seeds them levelled and checks every card in both modes; a new card belongs in it.
...
- **`TooltipTrigger` without `asChild` renders a `<button>`** (or the tag named by `as`), so an icon trigger is a role-`button` wrapper; Calculator's level/exp trigger, Tools' before/after previews (with a `tabIndex`) and Graph's two radio triggers are `as="div"`, so those are not buttons. Query inputs by placeholder, not role. React 19 reports nesting as `console.error("In HTML, %s cannot be a descendant of <%s>…")`; to check for it, spy on `console.error` and match `descendant`, since Vitest 5 defaults to `silent: "passed-only"` and hides console output from passing tests (`pnpm test --silent=false <filter>` shows it). React logs each distinct DOM warning **once per test file**, so only the first test that triggers it sees it; pair a console assertion with a DOM assertion.
- **Radios**: `RadioButton` is `role="radio"` inside a named `role="radiogroup"`; query `getByRole("radiogroup", { name: "Symbol type" })` (Selector) or `"X-axis spacing"` (Graph), then `getByRole("radio", { name })`. Arrow keys are tested with `fireEvent.keyDown(radio, { key: "ArrowDown" })` and assert `toHaveFocus()` plus `aria-checked`. Clicking the label text still works.
- **Nesting regression net**: `src/test/interactiveNesting.test.tsx` renders Header, Selector, Calculator, Tools, Overview (a row expanded) and Graph at desktop and tablet with the cap locked and unlocked, plus every Handbook tab, and fails on `button button`, `button input`, `button a`, `button [role=radio]` or a React DOM warning (nesting or `Received NaN`). Add a new card or trigger site to it; its `/next` blocks cover the redesign's calculator, handbook and changelog pages at desktop and phone widths.
...
- `src/test/dataWatcher.test.ts` (node environment): the data watcher's own copy of each server's numbers (`expectedFor` in `scripts/game-data.mjs`, plain JS imported through `allowJs`) equals `createInitialSymbols(region)` for all six servers, and its regional parsers (kiiten, BWIKI, the patch-note line filter) read fixtures cut from the real pages. The network half of `scripts/check-game-data.mjs` is not tested; its dry run is `pnpm check:data`.
