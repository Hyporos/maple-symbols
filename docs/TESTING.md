...
| Piece                                                                   | Version / note                                                                               |
| ----------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `vitest` + `@vitest/coverage-v8`                                        | 5.x (needs Vite ≥ 6.4 and Node ≥ 22.12; Vite is 7.3.1, Node 22.12.0 is exactly at the floor) |
| `jsdom`                                                                 | **pinned to 28.x**: 29+ require Node ≥ 22.13/22.22; bump when Node is upgraded               |
...
Known-good values (verified by `src/lib/utils.test.ts`; weekly credit follows `dayjs().day(8)`, see KI-003):
...
| Wed 2026-09-16 | `(120, 0, true)` / `(121, 0, true)`                  | 6 / 13 |
| Mon 2026-09-14 | `(120, 0, true)`                                     | 8      |
| Sat 2026-09-19 | `(120, 0, true)`                                     | 3      |
| Wed 2026-09-16 | `(200, 20, true)` / `(200, 20, false)`               | 6 / 10 |
...
- **Overview renders every row of the current mode plus each row's collapsed expander panel** (`hidden`, still in the DOM), so status strings such as "Indefinite" or "0" match up to seven times and `getByText` throws. Scope with `within(screen.getAllByAltText(name)[0].closest("button")!)`. The collapsed-row strings key on `daily`/`weekly`/`experience`, so Overview can be rendered alone, without Calculator's effect.
- **Store-writing effects** run on mount for the selected symbol; a Calculator test that seeds `level: 1, experience: 0, daily: true` will also write `symbolsRemaining`/`daysRemaining`/`completion` into the store. Freeze time if you assert on those.
...
## 8. Making more of the app unit-testable (suggestions, not done)

Extract into `src/lib/`: the overflow walk and remaining-to-max sum from `Calculator.tsx`; the selector/catalyst preview maths and `displayPreview` from `Tools.tsx`; Overview's nested-ternary label logic (the riskiest code in the app) into label functions; Graph's series/tick/target-date maths with an injected `now`. Give `advanceDayCount`/`calculateDaysRemaining` an optional `now` parameter. Export `normalize` from `RouterContext.tsx`. Each is behaviour-preserving and would replace a render-based test with a table-driven one.
