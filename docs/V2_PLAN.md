...
- **Grand Sacred symbols** (Tallahart, Geardrak): in 2.0 (Brian, 2026-09-16), in calculator, handbook and copy in the same release (SEO-11, SEO A-2). How they fit the data model is still open; what must be sourced first is listed in GAME §4.
...
- Grand Sacred symbols: a third family with no main stat, its own meso tables and no catalyst or selector (GAME §4) — how it fits `SymbolData` and the next-level panel.
