...
- **Grand Sacred symbols** (Tallahart, Geardock): in 2.0 (Brian, 2026-09-16), in calculator, handbook and copy in the same release (SEO-11, SEO A-2). The data model is built (2026-09-23: their own `grand` type, ids 13 and 14, REGIONS D-18); where they sit in the interface waits for the 2.0 visual design (GAME §4).
...
- **Visual redesign, Tasks 1–7** (2026-09-23): `/next` (`src/next/`) is a complete, deletable second UI behind `NEXT_UI` (dev and draft-serving builds only) — shell, calculator page, Handbook/Extras pages, and the `src/next/ui/` kit — sharing `lib`/`state`/`i18n`/`hooks` with the current UI (`docs/ARCHITECTURE.md` §2, `docs/DESIGN_SYSTEM.md` §12). Grand Sacred (Tallahart, Geardock) is selectable there for the first time. Full checks (`lint`, `typecheck`, `test`, `build`, plus a `SERVE_DRAFTS=1` build) are green; screenshots at 360/768/1280/1440 px, empty and seeded, are in `.playwright-mcp/redesign/` (git-ignored). One real bug turned up while extending `src/test/interactiveNesting.test.tsx` to cover `/next` (a `<div>` nested in a `<p>` in `GraphCard`'s Target `StatBox`): that coverage is written but not committed pending a fix decision (`.superpowers/sdd/2026-09-23-visual-redesign/task-7-report.md`). **User-visible**: none yet (`noindex`, not linked from the current UI, never prebuilt, never in a sitemap); a Vercel preview of `v2` serves it for review.
...
- Grand Sacred symbols: a third family with no main stat, its own meso tables, no Catalyst, and Sacred Symbol Selector eligibility for Tallahart only (since v.271, confirmed in game; GAME §4) — in `SymbolData` since 2026-09-23 (GAME §4); still open: where it sits in the interface, the next-level panel without main stat, and whether the Sacred Power total includes it.
