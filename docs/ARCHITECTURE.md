...
**Adding a route**: add the entry to `ROUTES` in `src/lib/routes.ts` (`path`, `title`, `description`, `nav` if it gets a Header link, `sitemap`) and widen `RoutePath` → add the `PageContent` branch in `App.tsx` (the `<SEO>` props come from the route already) → if it is an Extras tab, add it to `EXTRAS_TABS` and to the `activeFor` of the Extras nav entry → run `pnpm test seo` (the generated `index.html` pageMap and sitemap follow automatically).
