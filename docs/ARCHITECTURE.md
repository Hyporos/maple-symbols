...
The strings themselves exist once, in `routes.ts`: `SEO.tsx` defaults come from `routeFor("/")`, `sitemap.xml` is generated by `sitemapXml()` (emitted into `dist/` by the plugin and served by a dev middleware; there is no file under `public/`), and `robots.txt` points at it. `src/test/seo.test.tsx` checks the generated `index.html` and sitemap against `ROUTES` and renders the app at every route.
