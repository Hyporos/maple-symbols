...
- `vercel.json`: `buildCommand: pnpm build`, `trailingSlash: false`, SPA rewrite of every path to `/index.html`, 1-year immutable `Cache-Control` for `/assets/*` and the versioned `/fonts/*` (rename a font file to change it; everything else, HTML and `public/` images included, gets Vercel's default `max-age=0, must-revalidate`), `nosniff`/`SAMEORIGIN`/referrer/permissions headers, no CSP (Umami would need allow-listing). `source` patterns are path-to-regexp, not regex (SEO-20). Anything under `api/` would deploy as a Vercel function; the directory is empty.
