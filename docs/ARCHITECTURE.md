...
- **D12 `TooltipTrigger` renders its own anchor unless `asChild` gets one DOM element.** Icons and `RadioButton` cannot hold floating-ui's ref, so they sit in a plain trigger that renders a `<button>`; `as="div"` covers triggers around inputs or buttons (KI-007, resolved). The Handbook tables still reach the same `<button>` through the old `asChild` + `{" "}` workaround (the array child fails `isValidElement`); it works, but new code should not copy it.
