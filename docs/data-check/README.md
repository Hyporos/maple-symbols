...
Working files for re-confirming the game data (GAME §0 and §7 say what is confirmed, sourced or unverified). Each CSV is pre-filled with what the site uses now, so checking means correcting a cell, not typing a table. These are scratch files, not a source the app reads: once a sheet is settled, the numbers go into `src/lib/symbols.json`, `src/lib/game.ts` or `src/lib/ratioData.ts`, the result is recorded in GAME §7, and the sheet stays as the record of that check.
...
- Nothing here is authoritative until GAME §7 has a row for it.
...
| `resets.csv`                                     | Daily and weekly quest reset times, in UTC, per server (the input KI-013's fix needs)             | In game; official notices                                    |
