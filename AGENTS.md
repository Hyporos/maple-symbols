...
- Daily rate: `dailySymbols × (extra ? (arcane 2 : sacred 1.5) : 1)`, 0 when the daily toggle is off. Weekly quests exist only on arcane and add **240 per weekly reset**. Day counts start **tomorrow** (today's quests are assumed done) and the weekly lands on each counted **Thursday** (GMS reset day, confirmed 2026-09-22; other regions reset Monday), so a weekly-only answer is the distance to next Thursday. Extra exists only on Vanishing Journey and Chu Chu.
