...
- Daily rate: `dailySymbols × (extra ? (arcane 2 : sacred 1.5) : 1)`, 0 when the daily toggle is off. Weekly quests exist only on arcane and add **120 per Monday reset**. Day counts start **tomorrow** (today's quests are assumed done) and the weekly lands on each counted Monday, so a weekly-only answer is the distance to next Monday. Extra exists only on Vanishing Journey and Chu Chu.
...
4. **Never give a react-icons or `RadioButton` child to `<TooltipTrigger asChild>`**: they are function components that drop the ref, so floating-ui has no anchor. Put them in a plain `<TooltipTrigger>`, which renders its own `<button>`. `asChild` is for a single DOM element (`<button>`, `<input>`); a trigger around inputs or buttons uses `as="div"` so nothing interactive nests in a button (KI-007). The Handbook's `asChild` + `{" "}` sites are the old workaround for the same fallback; don't copy them.
