...
- 14 symbols: **arcane ids 1–6 (max level 20)**, **sacred ids 7–12 (max level 11)**, **grand ids 13–14** (Tallahart, Geardock: Sacred table, max 11, no weekly, extra, Catalyst or main stat; `null` in `game.ts` where a rule is absent). The store's `mode` (`SymbolType`) can be `"grand"` and `selectSymbol` accepts 13/14, but the current UI (`src/components/`) has no Grand tab: its lists read the family through `useMode()` (`Mode`: `"arcane"` | `"sacred"`, folding a grand selection back to sacred), so every one of its lists still filters on `symbol.type === mode`. Only `/next` (`src/next/`) offers Grand. `inFamily(type, family)` in `game.ts` is true for a family's own type and for `"grand"` inside `"sacred"`: `usePower` and the Graph's `buildDateSymbols` use it, so Grand Sacred's power counts toward the Sacred total everywhere (REGIONS D-18). Selection is by `id` (`selectedId`, with `lastSelected` remembered per family). JSON order only affects display order (gotcha 1).
