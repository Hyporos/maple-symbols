...
- Daily rate: `dailySymbols × (extra ? (arcane 2 : sacred 1.5) : 1)`, 0 when the daily toggle is off. Weekly quests exist only on arcane and add **240 per weekly reset**. Day counts start **tomorrow** (today's quests are assumed done) and the weekly lands on each counted **Thursday** (the reset day in every region since 2025–26; GAME §3), so a weekly-only answer is the distance to next Thursday. Extra exists only on Vanishing Journey and Chu Chu.
...
| ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| `src/lib/utils.ts`, `data.ts`, `game.ts`, `symbols.json`, `ratioData.ts`, `hooks/usePower.ts`   | Cheat sheet above; ARCHITECTURE §4; GAME §1–2 tables (the docs test fails) and §7 provenance log   |
