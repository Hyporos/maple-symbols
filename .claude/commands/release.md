...
5. **Storage**: if a player field in `src/lib/persistence.ts` changed meaning since the last tag, check `STORAGE_VERSION` was bumped with a conversion in `migrate`. Game-data changes in `symbols.json` need nothing: saves are restored by id onto fresh data (`docs/ARCHITECTURE.md` §3). Also confirm no existing symbol `id` changed.
