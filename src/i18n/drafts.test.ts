...
describe.each(DRAFT_LANGUAGES.filter((language) => DRAFT_CATALOGUES[language]))(
  "the %s draft (src/i18n/drafts.ts)",
  (language) => {
    const draft = DRAFT_CATALOGUES[language]!;
...
describe("drafts", () => {
  it("are only for the four translated editions' languages", () => {
    expect([...DRAFT_LANGUAGES].sort()).toEqual(
