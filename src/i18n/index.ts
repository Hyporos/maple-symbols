...
import { en } from "./en";
import { DEFAULT_NAME_SET, fillTerms, termsFor, type NameSet } from "./terms";
 ...
 * terms filled from `nameSet` and `{pageServer}` set to `pageServer`. Memoised, so every
 * call for the same page returns the same object.
 */
export function messagesFor(
  locale: string,
  nameSet: NameSet = DEFAULT_NAME_SET,
  pageServer: string = DEFAULT_EDITION.name
): Messages {
  const language = isLocale(locale) ? locale : (DEFAULT_LOCALE as Locale);
  const key = `${language}|${nameSet}|${pageServer}`;
  let messages = filled.get(key);
  if (messages === undefined) {
    messages = fillTerms(CATALOGUES[language], { ...termsFor(nameSet), pageServer });
...
export const editionMessages = (edition: Edition): Messages =>
  messagesFor(servedLocale(edition), nameSetFor(edition), edition.name);
