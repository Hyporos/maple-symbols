...
describe("Umami tracker tag", () => {
  ...
  it("allows exactly the canonical host from src/lib/routes.ts and its www form", () => {
    const hosts = html.match(/productionHosts = (\[[^\]]*\])/)?.[1];
    const apex = new URL(SITE_URL).hostname;
    expect(JSON.parse(hosts ?? "null")).toEqual([apex, `www.${apex}`]);
  });
